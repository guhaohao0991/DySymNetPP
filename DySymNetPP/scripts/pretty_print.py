...
def sym_pp(W_list, funcs, var_names, threshold=0.01, n_double=None,
    add_bias=False, biases=None):
    ...
    try:
        ...
        if add_bias and biases is not None:
            assert len(W_list) == len(biases
                ), 'The number of biases must be equal to the number of weights.'
            for i, (W, b) in enumerate(zip(W_list, biases)):
                ...
                expr = expr * W + b
                expr = apply_activation(expr, funcs[i + 1], n_double=
                    n_double[i])
        else:
...
def network(weights, funcs, var_names, threshold=0.01, add_bias=False,
    biases=None):
    ...
    Returns:
        Simplified sympy expression."""
    n_double = [functions.count_double(funcs_per_layer) for funcs_per_layer in
        funcs.values()]
    sp_funcs = {}
    ...
    if add_bias and biases is not None:
        assert len(weights) == len(biases
            ), 'The number of biases must be equal to the number of weights - 1.'
        expr = sym_pp(weights[:-1], sp_funcs, var_names, threshold=
            threshold, n_double=n_double, add_bias=add_bias, biases=biases[:-1]
            )
        expr = last_pp(expr, weights[-1], add_bias=add_bias, biases=biases[-1])
    else:
        expr = sym_pp(weights[:-1], sp_funcs, var_names, threshold=
            threshold, n_double=n_double, add_bias=add_bias)
...
def filter_expr(expr, threshold=0.01):
    ...
    for arg in expr.args:
        if arg.is_constant() and abs(arg) > threshold:
